...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """
        This migration is continuation of previous one (for unknown reason
        south did not see foreign keys and indexes just after creating new
        table)
        """

        # delete foreign keys - partitions does not allow foreign keys
        try:
        ...
        try:
            db.delete_index(u'ralph_scrooge_dailycost', 'pricing_object_id')
            db.delete_index(u'ralph_scrooge_dailycost', 'warehouse_id')
        except DatabaseError as e:
            print('Indexes not deleted', e)
        db.execute("ALTER TABLE `ralph_scrooge_dailycost` DROP PRIMARY KEY , ADD PRIMARY KEY (`id`, `date`, `forecast`)")
        ...
        db.execute("INSERT INTO `ralph_scrooge_dailycost` SELECT * FROM `ralph_scrooge_dailycost_backup`;")

    def backwards(self, orm):
        # table is deleted and renamred in this migration, so migration 0020_1
        # could see table ralph_scrooge_dailycost after rename
